...
"""
 The first set oflevels have students identify vulnerable reads and writesin a program that can be exploited. Memory leaking suchas with the Heartbleed vulnerability and memory corrup-tion  such  as  with  procedure-link  table  (PLT)  hijackingcan occur when adversarial input controls a pointer thatis used to either read from or write to memory.  To iden-tify this vulnerability, the symbolic execution engine canspecifically  look  for  unconstrained  or  symbolic  mem-ory reads and writes, that is, when a memory addressingmode uses a register that is symbolic to either read fromor write to memory.   In a first step towards leveragingsymbolic execution to automatically exploit vulnerabili-ties, levels that teach students how to set up the engine tolook for unconstrained memory access and how to thenconstrain  the  access  to  target  specific  locations  for  ex-ploitation are used to solve levels of the CTF.
"""
